...
// checkPreset(16);  // Adam commented this out - helped with auto populating bad settings
// fill in form
...
function changeURL() {
  ...
  // window.history.replaceState({}, '', url);  // Commented iout by Adam - so it doens't save parameters in a weird way upon refresh
...
// Adam Added
// Returns an array of 1 or 2 hex coords for a single touch point.
//  - Always returns the closest hex.
//  - Returns a second adjacent hex when the touch is near the boundary
//    (runner-up almost as close as the closest).
function getHexCoordsAtMulti(coords) {
  ...
  // Check neighbours for best + second-best distance
  var bestD = Infinity;
  var secondD = Infinity;
  var bestHex = new Point(q, r);
  var secondHex = null;
  ...
  // Only include 2nd if it's almost as close as the 1st (near border)
  if (secondHex && (secondD - bestD) <= deltaSq) {
    out.push(secondHex);
  }
...
function handleTouch(e) {
  ...
  // If keyboard keys or mouse are active, ignore touch
  ...
  // Mark everything for release, then "unmark" what is still touched
  for (var i = 0; i < settings.activeHexObjects.length; i++) {
    settings.activeHexObjects[i].release = true;
  }

  // De-dupe: the same hex can be produced twice (e.g., two touches near same boundary)
  var keep = {}; // key: "q,r" => true
  ...
  for (var i = 0; i < e.targetTouches.length; i++) {
    ...
    var coordsList = getHexCoordsAtMulti(p);

    for (var k = 0; k < coordsList.length; k++) {
      var coords = coordsList[k];
...
function checkPreset(init) {
  ...
  if (url_str.substr(0) == '.htm') { //url_str.length - 4
